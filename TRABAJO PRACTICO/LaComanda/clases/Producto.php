...
class Producto
{
	...
	function __construct($cod,$des,$pre)
    {
	  ...
	  {
			  $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			  $consulta =$objetoAccesoDato->RetornarConsulta("select codigo,descripcion, precio, stock from producto where descripcion = '$nombre'");
			  $consulta->execute();			
	  ...
	  public static function insertar($obj){
        $response = false;
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
          $consulta =$objetoAccesoDato->RetornarConsulta("INSERT into producto (codigo,descripcion,precio,stock)values('$obj->codigo','$obj->descripcion','$obj->precio','$obj->stock')");
        if($consulta->execute()){
            $response = true;
        }  		  
       return $response ;    
