...
class Pedido
{
	...
	public static function eliminarPedidos($idPed,$fec){
			$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta =$objetoAccesoDato->RetornarConsulta("DELETE from pedido WHERE id= '$idPed' AND fecha = '$fec'");	
			if($consulta->execute())
				return true ;
			else	
				return false ;
	...
	public static function v_listaPedidos(){
		$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta =$objetoAccesoDato->RetornarConsulta("SELECT fecha , Pedido As nroPedido, Cliente, Estado , Producto as Pedido  FROM v_pedidos");
			$consulta->execute();			
			return $consulta->fetchAll(PDO::FETCH_ASSOC) /*(PDO::FETCH_ASSOC)*/;	
	}
