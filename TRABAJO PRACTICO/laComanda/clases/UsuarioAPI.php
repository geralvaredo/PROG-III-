...
include("clases/IApiUsuario.php");
...
class UsuarioAPI extends Usuario implements IApiUsuario{
    ...
    public function insertarUsuario($request, $response){
        ...
        if($duplicado == NULL){   
            //$idPuesto = Puesto::traerIdPuesto($obj['puesto']);
    ...
    public function bajaUsuario ($request,$response){
        $obj = $request->getParsedBody();
        $clase = new stdclass();
         $usuario = Usuario::traeId($obj['email']);
         if($usuario != NULL){
            $usuarioBorrado = Usuario::borrarUsuario($obj['email'],$obj['clave']);
            if($usuarioBorrado){
    ...
    public function empleadoLogueo($request, $response){
        $personas = Usuario::EmpleadosHorarios();
